...
typedef struct {
    ...
    float strikeRate;    // Strike rate of the batsman
} Batsman;
...
// Function to calculate the current run rate
float calculateRunRate(int totalRuns, int overs, int balls) {
    float totalOvers = overs + (balls / 6.0);
    if (totalOvers == 0) return 0.0;
    return (float)totalRuns / totalOvers;
}

// Function to calculate the strike rate of a batsman
float calculateStrikeRate(int runs, int ballsPlayed) {
    if (ballsPlayed == 0) return 0.0;
    return (float)runs / ballsPlayed * 100;
}

// Function to play an innings
void playInnings(Batsman batsmen[], Bowler bowlers[], int *numBowlers, int maxOvers, int teamNumber, char teamName[]) {
    ...
    printf("Enter the name of the first batsman for %s: ", teamName);
    scanf("%s", batsmen[0].name);
    printf("Enter the name of the second batsman for %s: ", teamName);
    ...
    while(1) {
        ...
        float runRate = calculateRunRate(totalRuns, overs, balls);

        // Display current score, overs, run rate, and bowler
        printf("\nCurrent Score: %d/%d (Extras: %d)", totalRuns, wickets, extras);
        printf("\tOvers: %d.%d", overs, balls);
        printf("\tRun Rate: %.2f", runRate);
        ...
        switch(choice) {
            case 1: {
                ...
                if (runType == 1) {
                    ...
                    batsmen[striker].ballsPlayed++;
                    bowlers[*numBowlers - 1].runsConceded += runs;
                    balls++; // Update current bowler's runs conceded
                    ...
                    totalRuns += runs;
                    extras += runs;
                    batsmen[striker].ballsPlayed++; // Increment balls played by the batsman
                    balls++;
        ...
        if (wickets >= 10 || (maxOvers != 999 && overs >= maxOvers)) {
            printf("\nInnings Over!\n");
            break;
        }
    }
...
int main() {
    ...
    srand(time(NULL));
    printf("_______________________________________________USE UNDERSCORE IN PLACE OF SPACE TO AVOID ERROR_______________________________________________\n");
    ...
    // Prompt the user to enter team names
    char teamOneName[20], teamTwoName[20];
    printf("Enter the name of Team One: ");
    scanf("%s", teamOneName);
    printf("Enter the name of Team Two: ");
    scanf("%s", teamTwoName);
    ...
    int tossResult; // 0 for Heads, 1 for Tails
    char winningTeam[20];
    if (tossChoice == 1) {
        tossResult = rand() % 2; // Randomly generate 0 or 1
        printf("Toss Result: %s\n", tossResult == 0 ? "Heads" : "Tails");
        strcpy(winningTeam, tossResult == 0 ? teamOneName : teamTwoName);
        printf("%s won the toss!\n", winningTeam);
    } else if (tossChoice == 2) {
        printf("Enter the name of the team that won the toss: ");
        scanf("%s", winningTeam);
        if (strcmp(winningTeam, teamOneName) == 0) {
            tossResult = 0; // Team One wins
            printf("%s won the toss!\n", teamOneName);
        } else if (strcmp(winningTeam, teamTwoName) == 0) {
            tossResult = 1; // Team Two wins
            printf("%s won the toss!\n", teamTwoName);
        } else {
            printf("Invalid team name! Defaulting to Team One winning the toss.\n");
            tossResult = 0;
            strcpy(winningTeam, teamOneName);
            printf("%s won the toss!\n", teamOneName);
        }
    } else {
        printf("Invalid choice. Defaulting to Team One winning the toss.\n");
        tossResult = 0;
        strcpy(winningTeam, teamOneName);
        printf("%s won the toss!\n", teamOneName);
    }

    // Ask the toss-winning team to choose batting or bowling
    int battingChoice;
    printf("%s, choose:\n1. Batting\n2. Bowling\nEnter your choice: ", winningTeam);
    scanf("%d", &battingChoice);

    // Determine the batting and bowling order based on the toss result
    char *battingTeam, *bowlingTeam;
    if ((tossResult == 0 && battingChoice == 1) || (tossResult == 1 && battingChoice == 2)) {
        battingTeam = teamOneName;
        bowlingTeam = teamTwoName;
    } else {
        battingTeam = teamTwoName;
        bowlingTeam = teamOneName;
    }

    // Prompt the user to choose overs type
    int maxOvers;
    printf("Select overs type:\n1. Unlimited\n2. Defined\nEnter your choice: ");
    int oversChoice;
    scanf("%d", &oversChoice);

    if (oversChoice == 1) {
        maxOvers = 999; // Represents unlimited
    } else if (oversChoice == 2) {
        printf("Enter the maximum number of overs: ");
        scanf("%d", &maxOvers);
    } else {
        printf("Invalid choice. Defaulting to defined overs.\n");
        printf("Enter the maximum number of overs: ");
        scanf("%d", &maxOvers);
    }

    // Play the first innings (batting team)
    printf("\n%s's Innings:\n", battingTeam);
    playInnings(tossResult == 0 ? teamOneBatsmen : teamTwoBatsmen, tossResult == 0 ? teamTwoBowlers : teamOneBowlers, tossResult == 0 ? &numTeamTwoBowlers : &numTeamOneBowlers, maxOvers, 1, battingTeam);

    // Play the second innings (bowling team)
    printf("\n%s's Innings:\n", bowlingTeam);
    playInnings(tossResult == 0 ? teamTwoBatsmen : teamOneBatsmen, tossResult == 0 ? teamOneBowlers : teamTwoBowlers, tossResult == 0 ? &numTeamOneBowlers : &numTeamTwoBowlers, maxOvers, 2, bowlingTeam);
