...
void playInnings(Batsman batsmen[], Bowler bowlers[], int *numBowlers, int maxOvers, char teamName[]) {
    ...
    while(1) {
        ...
        printf("\nCurrent Score: %s - %d/%d (Extras: %d)", teamName, totalRuns, wickets, extras);
    ...
    printf("Total: %s - %d/%d (Extras: %d) in %d.%d overs\n", teamName, totalRuns, wickets, extras, overs, balls);
    printf("\nBatsmen:\n");
    for (int i = 0; i < 11; i++) {
...
int main() {
    ...
    printf("\n%s's Innings:\n", teamOneName);
    playInnings(teamOneBatsmen, teamOneBowlers, &numTeamOneBowlers, maxOvers, teamOneName);

    // Play Team Two's innings
    printf("\n%s's Innings:\n", teamTwoName);
    playInnings(teamTwoBatsmen, teamTwoBowlers, &numTeamTwoBowlers, maxOvers, teamTwoName);
